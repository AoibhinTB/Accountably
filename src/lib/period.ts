...
// More verbose / explicit version of timeAgo for note timestamps:
// minutes-since for the first hour, hours-since up to 24h, then an absolute
// short date afterwards. Older notes carry a fixed date so the page does not
// keep re-counting (and the "edit within 24h" affordance lines up neatly).
export function noteTimestamp(iso: string, now: Date = new Date()): string {
  const diffMs = now.getTime() - new Date(iso).getTime();
  if (diffMs < 60_000) return "just now";
